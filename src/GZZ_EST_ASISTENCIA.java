...
public class GZZ_EST_ASISTENCIA extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel6.setText("Tabla_Asistencia");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(CarFlaAct == 1) {
            ...
            try {
                ...
                switch(comandoActivo) {      
                    case 1 -> { 
                        stmt = conn.prepareStatement("INSERT INTO GZZ_EST_ASISTENCIA (EstAsiCod, EstAsiDes, EstRegCod) VALUES (?, ?, ?)");
                        stmt.setString(1, codigo);
                        stmt.setString(2, descripcion);
                        stmt.setString(3, estadoRegistro);
                    }
                    case 2 -> { 
                        stmt = conn.prepareStatement("UPDATE GZZ_EST_ASISTENCIA SET EstAsiDes = ? WHERE EstAsiCod = ?");
                        stmt.setString(1, descripcion);
                        stmt.setString(2, codigo);
                    }
                    default -> {
                        stmt = conn.prepareStatement("UPDATE GZZ_EST_ASISTENCIA  SET EstRegCod = ? WHERE EstAsiCod = ?");
    ...
    private void llenarTablaCargo() {        
        ...
        try {
            ...
            //Agregando los datos
            while(rs.next()) {
                modelo.addRow(new Object[]{rs.getString("EstAsiCod"), rs.getString("EstAsiDes"), rs.getString("EstRegCod")});
            }
