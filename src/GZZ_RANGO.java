...
public class GZZ_RANGO extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Registro de Rango");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(CarFlaAct == 1) {
            ...
            try {
                ...
                switch(comandoActivo) {      
                    case 1 -> { 
                        stmt = conn.prepareStatement("INSERT INTO GZZ_RANGO (RanCod, RanDes, EstRegCod) VALUES (?, ?, ?)");
                        stmt.setString(1, codigo);
                        stmt.setString(2, descripcion);
                        stmt.setString(3, estadoRegistro);
                    }
                    case 2 -> { 
                        stmt = conn.prepareStatement("UPDATE GZZ_RANGO SET RanDes = ? WHERE RanCod = ?");
                        stmt.setString(1, descripcion);
                        stmt.setString(2, codigo);
                    }
                    default -> {
                        stmt = conn.prepareStatement("UPDATE GZZ_RANGO SET EstRegCod = ? WHERE RanCod = ?");
    ...
    private void llenarTablaCargo() {        
        ...
        try {
            ...
            String consulta = "SELECT RanCod, RanDes, EstRegCod FROM GZZ_RANGO WHERE EstRegCod NOT IN ('*')";
            ResultSet rs = statement.executeQuery(consulta);
            
            //Agregando los datos
            while(rs.next()) {
                modelo.addRow(new Object[]{rs.getString("RanCod"), rs.getString("RanDes"), rs.getString("EstRegCod")});
            }
