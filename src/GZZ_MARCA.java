...
public class GZZ_MARCA extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Registro de Marca");
        ...
        tablaMarca.setShowHorizontalLines(true);
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(CarFlaAct == 1) {
            ...
            try {
                ...
                switch(comandoActivo) {      
                    case 1 -> { 
                        stmt = conn.prepareStatement("INSERT INTO GZZ_MARCA (MarCod, MarNom, EstRegCod) VALUES (?, ?, ?)");
                        stmt.setString(1, codigo);
                        stmt.setString(2, descripcion);
                        stmt.setString(3, estadoRegistro);
                    }
                    case 2 -> { 
                        stmt = conn.prepareStatement("UPDATE GZZ_MARCA SET MarNom = ? WHERE MarCod = ?");
                        stmt.setString(1, descripcion);
                        stmt.setString(2, codigo);
                    }
                    default -> {
                        stmt = conn.prepareStatement("UPDATE GZZ_MARCA SET EstRegCod = ? WHERE MarCod = ?");
    ...
    private void btnModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificarActionPerformed
        int filaSeleccionada = tablaMarca.getSelectedRow();
    ...
    private void llenarTablaCargo() {        
        modelo.setColumnIdentifiers(new Object[]{"Codigo", "Nombre", "Estado Registro"});
        modelo.setRowCount(0);
        try {
            //Consulta de selección
            Statement statement = conn.createStatement();
            String consulta = "SELECT MarCod, MarNom, EstRegCod FROM GZZ_MARCA WHERE EstRegCod NOT IN ('*')";
            ResultSet rs = statement.executeQuery(consulta);
            
            //Agregando los datos
            while(rs.next()) {
                modelo.addRow(new Object[]{rs.getString("MarCod"), rs.getString("MarNom"), rs.getString("EstRegCod")});
            }
