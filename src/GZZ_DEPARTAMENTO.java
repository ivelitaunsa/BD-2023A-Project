...
public class GZZ_DEPARTAMENTO extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Registro de Departamento");
        ...
        jLabel6.setText("Tabla_Departamento");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(CarFlaAct == 1) {
            ...
            try {
                ...
                switch(comandoActivo) {      
                    case 1 -> { 
                        stmt = conn.prepareStatement("INSERT INTO GZZ_DEPARTAMENTO (DepCod, DepDes, EstRegCod) VALUES (?, ?, ?)");
                        stmt.setString(1, codigo);
                        stmt.setString(2, descripcion);
                        stmt.setString(3, estadoRegistro);
                    }
                    case 2 -> { 
                        stmt = conn.prepareStatement("UPDATE GZZ_DEPARTAMENTO SET DepDes = ? WHERE DepCod = ?");
                        stmt.setString(1, descripcion);
                        stmt.setString(2, codigo);
                    }
                    default -> {
                        stmt = conn.prepareStatement("UPDATE GZZ_DEPARTAMENTO SET EstRegCod = ? WHERE DepCod = ?");
    ...
    private void llenarTablaDepartamento() {        
        modelo.setColumnIdentifiers(new Object[]{"Codigo", "Descripcion", "Estado Registro"});
        modelo.setRowCount(0);
        try {
            //Consulta de selección
            Statement statement = conn.createStatement();
            String consulta = "SELECT DepCod, DepDes, EstRegCod FROM GZZ_DEPARTAMENTO WHERE EstRegCod NOT IN ('*')";
            ResultSet rs = statement.executeQuery(consulta);
            
            //Agregando los datos
            while(rs.next()) {
                modelo.addRow(new Object[]{rs.getString("DepCod"), rs.getString("DepDes"), rs.getString("EstRegCod")});
            }
